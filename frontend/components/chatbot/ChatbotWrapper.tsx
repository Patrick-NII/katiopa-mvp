...
const ChatBubble = dynamic(() => import('./ChatBubble'), { 
  ssr: false,
  loading: ({ subscriptionType = 'FREE' }) => {
    const getLoadingColors = () => {
      if (subscriptionType === 'PRO_PLUS' || subscriptionType === 'ENTERPRISE') {
        return 'from-orange-500 to-violet-600'
      } else if (subscriptionType === 'PRO') {
        return 'from-violet-600 to-pink-500'
      } else {
        return 'from-blue-600 to-purple-600'
      }
    }
    return (
      <div className={`fixed bottom-6 right-6 w-16 h-16 bg-gradient-to-r ${getLoadingColors()} rounded-full shadow-2xl flex items-center justify-center text-white`}>
