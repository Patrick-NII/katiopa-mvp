// Intégration des données CubeMatch dans Bubix pour l'IA
...
// Fonction pour intégrer les données CubeMatch dans le contexte utilisateur
export async function integrateCubeMatchIntoUserContext(userId: string, existingContext: any): Promise<any> {
  try {
    const cubeMatchData = await getCubeMatchData(userId);
    
    if (!cubeMatchData) {
      return existingContext;
    }

    const cubeMatchSummary = generateCubeMatchSummary(cubeMatchData);

    return {
      ...existingContext,
      cubeMatchData,
      cubeMatchSummary,
      hasCubeMatchData: true
    };
  } catch (error) {
    console.error('Erreur lors de l\'intégration des données CubeMatch:', error);
    return existingContext;
  }
}
